...
for n in range(1,timerange):
    ...
    for i in range(B):
        ...
        for k in range(len(r)):
            ...
            if use_pitch_controller:
                if i==0:
                    aoa_deg = np.rad2deg(phi) - (beta_deg[k] + pnew + np.rad2deg(theta_p_arr[n-1]))
                else:
                    aoa_deg = np.rad2deg(phi) - (beta_deg[k] + np.rad2deg(theta_p_arr[n-1]))
            else:
                if i==0:
                    aoa_deg = np.rad2deg(phi) - (beta_deg[k] + pnew + np.rad2deg(theta_p))
                else:
    ...
    theta_p = amplitude * np.sin(omega_pitch * time_arr[n])
...
if plot_deflection:
    if use_dof3:
        ...
        plt.xlim(0,4)
        #plt.ylim(0,50)
        plt.title('Power spectral density of deflection')
    ...
    if use_dof11:
        ...
        #Plotting only the last blade element
        # plt.plot(time_arr, uz[-1, 0, :], label = 'Flapwise tip deflection')
        # plt.plot(time_arr, uy[-1, 0, :], label = 'Edgewise tip deflection')
        
        plt.plot(time_arr[mask], uz[-1, 0, mask], label = 'Flapwise tip deflection')
        plt.plot(time_arr[mask], uy[-1, 0, mask], label = 'Edgewise tip deflection')
        ...
        if plot_bending_moment:
            ...
            plt.plot(M_blade1_edge_freq, M_blade1_edge_psd*10**(-6), color='darkorange', label='Edgewise bending moment')    
            plt.plot(M_blade1_flap_freq, M_blade1_flap_psd*10**(-6),label='Flapwise bending moment')
            ax.set( xlabel = '$f [Hz]', ylabel = 'PSD [$(MN\cdot m)^{2} / Hz$]')
            ax.set_title('Power spectral density of bending moment')
            ax.set_xlim(0, 4)
        ...
        if plot_tower_deflection:
            ...
            ################################# PSD plot af tower deflection #####################3
            sec_to_dis = 150
            # observations to discard
            obs_to_dis = int(sec_to_dis/delta_t)
...
#%% Turbulens loading plots specific element
...
#%% Turbulens plots, PSD of total thrust
...
if plot_thrust_psd:
    ...
    fig,ax=plt.subplots(1,1)
    plt.plot(T_freq*2*np.pi/omega_arr[-1], T_psd/(10**6), label='Blade')
    ...
    #Compute and plot the power spectral density. 
    T_freq, P_psd = signal.welch(P_arr[obs_to_dis:], fs, nperseg=1024)
    fig,ax=plt.subplots(1,1)
    plt.plot(T_freq*2*np.pi/omega_arr[-1], P_psd/(10**6), label='Blade')
    # plt.axvline(3,color='black')
    ax.set(xlim = [0,10], xlabel = '$2 \pi f / \omega}$ [-]', ylabel = 'PSD [$(MW)^{2} / Hz$]')
    # Sætter y lim, så den er lidt højere end peaket
    ylim_filter = (T_freq*2*np.pi/omega) > 1
    # ax.set_ylim(0,T_psd[ylim_filter].max()*1.1/10**6)
    ax.set_title('Power spectral density of total power')
    ax.grid()
    plt.show()

#%% Pitch angle as function of windspeed
"""
